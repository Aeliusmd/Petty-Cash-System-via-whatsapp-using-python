...
async def process_message(correlation_id: str, payload: dict, session: str):
    ...
    try:
        ...
        message_text = payload.get('body') or payload.get('text') or payload.get('message') or ''
        ...
        has_media = payload.get('hasMedia') or payload.get('media') or payload.get('_data', {}).get('media')
        media_type = payload.get('type') or payload.get('messageType') or payload.get('_data', {}).get('type')
        mime_type = payload.get('mimetype') or payload.get('_data', {}).get('mimetype', '')

        # Media data from WAHA
        media_data = (payload.get('media', {}).get('data') or 
                      payload.get('_data', {}).get('media', {}).get('data') or
                      payload.get('mediaData'))
        media_url = (payload.get('media', {}).get('url') or 
                     payload.get('mediaUrl') or 
                     payload.get('_data', {}).get('mediaUrl'))
        media_filename = (payload.get('media', {}).get('filename') or
                          payload.get('_data', {}).get('media', {}).get('filename'))
...
async def waha_webhook(request: Request, background_tasks: BackgroundTasks):
    ...
    # Always return 200 immediately to prevent WAHA retry storms
    response = {"received": True, "correlationId": correlation_id}
    ...
    background_tasks.add_task(process_message, correlation_id, payload, session)
